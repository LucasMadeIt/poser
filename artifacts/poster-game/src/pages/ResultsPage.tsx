...
export function ResultsPage({ room, myPlayerId, amIHost, onPlayAgain }: Props) {
  ...
  return (
    <div style={{ position:"relative", minHeight:"100vh", overflow:"hidden" }}>
      ...
      {/* Header */}
      <div style={{ position:"relative", zIndex:20, background:"rgba(10,8,6,0.92)", borderBottom:`2px solid ${NAVY}55`, padding:"0.6rem 1.25rem", display:"flex", alignItems:"center", gap:"1rem", backdropFilter:"blur(4px)" }}>
        <img src="/poster-logo.png" alt="POSTER" style={{ height:38, display:"block", objectFit:"contain" }} />
        <div style={{ fontFamily:BEBAS, letterSpacing:"0.15em", fontSize:"1.15rem", color: isEnded ? ORANGE : MUSTARD }}>
        ...
        <div style={{ width:"100%", maxWidth:620, display:"flex", flexDirection:"column", gap:"1.4rem" }}>
          ...
          {latestResult && (
            <div style={{ position:"relative" }}>
              <div style={{ position:"relative", background:CREAM, padding:"2rem 2.2rem 1.8rem", transform:"rotate(-0.7deg)", boxShadow:"8px 14px 48px rgba(0,0,0,0.8)" }}>
                <TapeH color={ORANGE} width={110} />
                <div style={{ fontFamily:BEBAS, fontSize:"0.7rem", letterSpacing:"0.28em", color:"#8A7050", marginBottom:"0.85rem" }}>
                  THE IMPOSTER WAS…
                </div>
                {imposter && (
                  <div style={{ display:"flex", alignItems:"center", gap:"1.25rem", marginBottom:"1.5rem" }}>
                    <div style={{ width:60, height:60, borderRadius:"50%", background:imposter.color, boxShadow:`0 0 24px ${imposter.color}88, 0 4px 16px rgba(0,0,0,0.5)`, flexShrink:0, border:"3px solid rgba(0,0,0,0.15)" }} />
                    <div style={{ flex:1 }}>
                      ...
                      {imposter.id === myPlayerId && (
                        <div style={{ fontFamily:DM, fontSize:"0.85rem", color:ORANGE, fontWeight:700, marginTop:2 }}>That was you!</div>
                      )}
                    </div>
                    <div style={{
                      fontFamily:BEBAS, fontSize:"2rem", lineHeight:1, letterSpacing:"0.04em",
                      color: latestResult.caught ? ORANGE : TEAL,
                      background: latestResult.caught ? `${ORANGE}14` : `${TEAL}14`,
                      border: `3px solid ${latestResult.caught ? ORANGE : TEAL}`,
              ...
              <svg viewBox="0 0 620 18" style={{ display:"block", width:"100%", marginTop:-2, filter:"drop-shadow(0 4px 8px rgba(0,0,0,0.35))" }} preserveAspectRatio="none">
                <path d="M0 0 L0 7 Q30 16 60 7 Q90 0 120 13 Q150 18 180 7 Q210 0 240 14 Q270 18 300 7 Q330 0 360 13 Q390 18 420 7 Q450 0 480 13 Q510 18 540 7 Q570 0 600 13 Q615 16 620 9 L620 0 Z" fill={CREAM}/>
              </svg>
            </div>
          )}
          ...
          {room.votes && Object.keys(room.votes).length > 0 && (
            <div style={{ position:"relative" }}>
              <div style={{ background:"#1A1410", border:"3px solid #0A0906", boxShadow:"6px 8px 0 rgba(0,0,0,0.7)", padding:"1.25rem 1.5rem", transform:"rotate(0.5deg)", position:"relative" }}>
                <TapeCorner color={NAVY} corner="tl" />
                  ...
                  {room.players.map((player) => {
                    ...
                    return (
                      <div key={player.id} style={{ display:"flex", alignItems:"center", gap:"0.7rem" }}>
                        ...
                        <span style={{ fontFamily:DM, fontSize:"0.85rem", color: isImp ? CREAM : "#8A7050", flex:1, fontWeight: isImp ? 700 : 400 }}>
                        ...
                        <div style={{ display:"flex", gap:"4px", alignItems:"center" }}>
                          ...
                          {votes === 0 && <span style={{ fontFamily:DM, fontSize:"0.7rem", color:"rgba(237,229,204,0.18)" }}>no votes</span>}
                        </div>
                      </div>
                    );
                  })}
          ...
          {room.results.length > 1 && (
            <div style={{ background:"rgba(14,10,6,0.72)", border:"1px solid rgba(237,229,204,0.08)", padding:"1rem 1.25rem", backdropFilter:"blur(4px)" }}>
              ...
              {room.results.map((r) => {
                ...
                return (
                  <div key={r.round} style={{ display:"flex", alignItems:"center", gap:"0.75rem", paddingBottom:"0.3rem" }}>
                    ...
                    <span style={{ fontFamily:DM, fontSize:"0.78rem", color:"#8A7050", flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{r.prompt}</span>
          ...
          {isEnded ? (
            <div style={{ position:"relative" }}>
              <div style={{ background:"#1A1410", border:`4px solid ${ORANGE}`, boxShadow:`8px 8px 0 ${NAVY}`, padding:"2rem", textAlign:"center", transform:"rotate(-0.5deg)", position:"relative" }}>
                <TapeH color={ORANGE} width={120} />
                <div style={{ fontFamily:BEBAS, fontSize:"3.5rem", color:CREAM, letterSpacing:"0.06em", marginBottom:"0.4rem", lineHeight:1 }}>GAME OVER</div>
                <div style={{ fontFamily:DM, fontSize:"0.9rem", color:"#8A7050", marginBottom:"1.5rem" }}>
                ...
                <div style={{ display:"flex", justifyContent:"center", gap:"1.2rem", marginBottom:"1.75rem", flexWrap:"wrap" }}>
                  {room.players.map((p) => (
                    <div key={p.id} style={{ display:"flex", alignItems:"center", gap:"0.4rem" }}>
                      ...
                      <span style={{ fontFamily:DM, fontSize:"0.85rem", color:CREAM }}>{p.name}</span>
                    </div>
                  ))}
                </div>
                {amIHost ? (
                  <button
                    onClick={onPlayAgain}
                    style={{ fontFamily:BEBAS, letterSpacing:"0.12em", fontSize:"1.6rem", color:CREAM, background:ORANGE, border:`2px solid #8A3008`, boxShadow:"5px 5px 0 rgba(0,0,0,0.75)", padding:"0.5rem 3rem", cursor:"pointer", transition:"transform 0.1s, box-shadow 0.1s" }}
                  ...
                  <div style={{ fontFamily:DM, fontSize:"0.8rem", color:"rgba(237,229,204,0.32)" }}>Waiting for host to start a new game…</div>
                )}
              </div>
            </div>
          ) : (
            <div style={{ textAlign:"center", fontFamily:BEBAS, fontSize:"0.9rem", letterSpacing:"0.15em", color:"#3a3020" }}>NEXT ROUND STARTING SOON…</div>
          )}
