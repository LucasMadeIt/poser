...
// ─── Local transform (drag + resize) ────────────────────────────────────────
type LocalTransform = { x: number; y: number; w: number; h: number };

// ─── Chip sections ───────────────────────────────────────────────────────────
const SECTIONS: SectionDef[] = [
  {
    id: "frames", label: "Frames  (F)", defaultOpen: true,
    chips: [
      { label: "Web Frame", type: "frameweb", defaults: { width: 560, height: 360, fill: "#ffffff" }, preview: () => <div style={{ width: "100%", border: "1.5px solid #555", borderRadius: 5, overflow: "hidden" }}><div style={{ height: 10, background: "#2a2a2a", display: "flex", alignItems: "center", gap: 3, padding: "0 4px" }}>{["#f56","#fa0","#2c2"].map(c => <div key={c} style={{ width: 4, height: 4, borderRadius: "50%", background: c }} />)}</div><div style={{ height: 18, background: "#1a1a1a", padding: "2px 4px" }}>{[60,40,30].map((w,i) => <div key={i} style={{ height: 2, background: "#444", borderRadius: 1, width: `${w}%`, marginBottom: 2 }} />)}</div></div> },
      { label: "Mobile Frame", type: "framemobile", defaults: { width: 200, height: 380, fill: "#ffffff" }, preview: () => <div style={{ width: 40, height: 64, border: "3px solid #555", borderRadius: 10, background: "#1a1a1a", position: "relative", overflow: "hidden", margin: "0 auto" }}><div style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)", width: "40%", height: 6, background: "#333", borderRadius: "0 0 4px 4px" }} /><div style={{ position: "absolute", inset: "8px 2px 6px", background: "#F8F4EE", borderRadius: 4 }} /><div style={{ position: "absolute", bottom: 2, left: "50%", transform: "translateX(-50%)", width: "30%", height: 2, background: "#444", borderRadius: 2 }} /></div> },
    ],
  },
  {
    id: "shape", label: "Shapes & Text  (R / T)", defaultOpen: true,
    chips: [
      ...
      { label: "Body Text", type: "text", defaults: { width: 240, height: 72, content: "Body text here.", fill: "#555", fontSize: 14 }, preview: () => <div style={{ display: "flex", flexDirection: "column", gap: 3, width: "100%", padding: "0 4px" }}>{[100,82,65].map((w,i) => <div key={i} style={{ height: 2.5, background: "#666", borderRadius: 2, width: `${w}%` }} />)}</div> },
      { label: "Caption", type: "text", defaults: { width: 180, height: 24, content: "Small caption", fill: "#888", fontSize: 11 }, preview: () => <div style={{ height: 2, background: "#666", borderRadius: 1, width: "55%", margin: "0 auto" }} /> },
  ...
  },
  {
    ...
    chips: [
      { label: "Nav Bar", type: "navbar", defaults: { width: 600, height: 60, fill: "#ffffff" }, preview: () => <div style={{ width: "100%", height: 20, background: "#1e1e1e", borderRadius: 3, display: "flex", alignItems: "center", padding: "0 6px", gap: 8 }}><div style={{ width: 24, height: 6, background: "#555", borderRadius: 2 }} /><div style={{ flex: 1 }} />{[0,1,2].map(i => <div key={i} style={{ width: 16, height: 4, background: "#444", borderRadius: 2 }} />)}<div style={{ width: 12, height: 12, borderRadius: "50%", background: "#555" }} /></div> },
      ...
      { label: "Sidebar", type: "sidebar", defaults: { width: 200, height: 320, fill: "#f7f7f7" }, preview: () => <div style={{ width: "100%", height: 32, border: "1.5px solid #444", borderRadius: 3, overflow: "hidden", display: "flex" }}><div style={{ width: "40%", background: "#222", display: "flex", flexDirection: "column", gap: 3, padding: 3 }}>{[0,1,2].map(i => <div key={i} style={{ height: 3, background: "#444", borderRadius: 2, width: "80%" }} />)}</div><div style={{ flex: 1, background: "#2a2a2a" }} /></div> },
...
export function GamePage({ room, myPlayerId, amIHost, onAdd, onUpdate, onDelete, onSkip, remoteCursors, emitCursorMove }: Props) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "z") {
        ...
        if (action?.type === "move") onUpdate(action.id, { x:action.oldX, y:action.oldY, width:action.oldW, height:action.oldH });
      }
      ...
      // Quick-add shortcuts (no modifier, no meta)
      if (!e.ctrlKey && !e.metaKey && !e.altKey) {
        if (e.key === "f" || e.key === "F") { e.preventDefault(); quickAdd("frameweb", 560, 360); }
  ...
  function quickAdd(type: CanvasElement["type"], w: number, h: number, content?: string) {
    ...
    onAdd({ type, x, y, width:w, height:h, fill: type==="frameweb"?"#ffffff":type==="rect"?"#e8e8e8":type==="text"?"#333333":"#1a1a1a", content, fontSize:type==="text"?16:undefined });
  }
  ...
  const handleCanvasMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (draggingRef.current && !resizingRef.current) {
      ...
      setLocalTransforms((p) => ({
        ...p,
        [elId]: { x:Math.max(0,Math.min(CANVAS_W-el.width,origX+dx)), y:Math.max(0,Math.min(CANVAS_H-el.height,origY+dy)), w:el.width, h:el.height },
      }));
    }
    ...
    if (resizingRef.current) {
      ...
      nx = Math.max(0, Math.min(CANVAS_W - MIN_W, nx));
      ny = Math.max(0, Math.min(CANVAS_H - MIN_H, ny));
  ...
  const handleCanvasMouseUp = useCallback(() => {
    ...
    if (resizingRef.current) {
      ...
      const lt = localTransforms[elId];
      if (lt) {
        onUpdate(elId, { x:Math.round(lt.x), y:Math.round(lt.y), width:Math.round(lt.w), height:Math.round(lt.h) });
        if (undoStackRef.current.length >= 10) undoStackRef.current.shift();
        undoStackRef.current.push({ type:"move", id:elId, oldX:origX, oldY:origY, oldW:origW, oldH:origH });
      }
  ...
  const toolbarY = selectedEl && selectedLT
    ? (selectedLT.y < TOOLBAR_H + 16 ? selectedLT.y + selectedLT.h + 8 : selectedLT.y - TOOLBAR_H - 10)
    : 0;
  const toolbarX = selectedEl && selectedLT
    ? Math.max(4, Math.min(CANVAS_W - 320, selectedLT.x + selectedLT.w / 2 - 160))
    : 0;

  // Alignment helpers (to canvas bounds)
  function alignEl(axis: "lx"|"cx"|"rx"|"ty"|"cy"|"by") {
    ...
    let nx=lt.x, ny=lt.y;
    if (axis==="lx") nx=0;
    if (axis==="cx") nx=Math.round((CANVAS_W-lt.w)/2);
    if (axis==="rx") nx=CANVAS_W-lt.w;
    if (axis==="ty") ny=0;
    if (axis==="cy") ny=Math.round((CANVAS_H-lt.h)/2);
    if (axis==="by") ny=CANVAS_H-lt.h;
    ...
    <div style={{ height:"100vh", display:"flex", flexDirection:"column", background:"#1a1a1a", overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        <div style={{ width:PANEL_W, background:"#212121", borderRight:"1px solid #1a1a1a", display:"flex", flexDirection:"column", flexShrink:0 }}>
          ...
          {/* Players */}
          <div style={{ borderTop:"1px solid #2a2a2a", padding:"0.5rem 0.8rem 0.55rem" }}>
            <div style={{ fontFamily:GRUNGE, fontSize:"0.58rem", letterSpacing:"0.15em", color:"#555", marginBottom:"0.3rem" }}>PLAYERS</div>
            <div style={{ display:"flex", flexDirection:"column", gap:"0.25rem" }}>
              {room.players.map((p) => (
                <div key={p.id} style={{ display:"flex", alignItems:"center", gap:"0.5rem" }}>
                  ...
                  <span style={{ fontFamily:DM, fontSize:"0.69rem", color:p.id===myPlayerId?"#E8E2D9":"#555", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", fontWeight:p.id===myPlayerId?600:400 }}>
                    {p.name}{p.id===myPlayerId?" (you)":""}
                  </span>
                </div>
              ))}
            </div>
            <div style={{ fontFamily:DM, fontSize:"0.56rem", color:"#3a3a3a", marginTop:"0.4rem", lineHeight:1.55 }}>
              F=Frame · R=Rect · T=Text<br />Drag corner to resize · Ctrl+D dupe
            </div>
          </div>
        </div>
        ...
        <div style={{ flex:1, overflow:"auto", display:"flex", alignItems:"center", justifyContent:"center", background:"#1a1a1a", padding:"3rem 5rem 2rem" }}>
          <div style={{ position:"relative" }}>
            ...
            <div style={{ position:"absolute", top:168, right:-144, width:124, background:"#f0f0f0", padding:"8px 10px 10px", transform:"rotate(-2deg)", boxShadow:"2px 4px 10px rgba(0,0,0,0.4)", zIndex:5, pointerEvents:"none" }}>
              <div style={{ fontFamily:GRUNGE, fontSize:"0.56rem", color:"#888", letterSpacing:"0.1em", marginBottom:4 }}>SHORTCUTS</div>
              <div style={{ fontFamily:DM, fontSize:"0.59rem", color:"#333", lineHeight:1.7 }}>F — Frame<br />R — Rectangle<br />T — Text<br />Ctrl+D — Dupe<br />Ctrl+Z — Undo</div>
            </div>
            ...
            >
              ...
              {selectedEl && selectedLT && !editingId && (() => {
                const H = 9;
                const ex = selectedLT.x;
                const ey = selectedLT.y;
                const ew = selectedLT.w;
                const eh = selectedLT.h;
                const CURSORS = ["nwse-resize","nesw-resize","nesw-resize","nwse-resize"] as const;
                // handle positions: 0=TL, 1=TR, 2=BL, 3=BR
                const handles: [number,number,0|1|2|3][] = [
                  [ex-H/2, ey-H/2, 0],
                  [ex+ew-H/2, ey-H/2, 1],
                  [ex-H/2, ey+eh-H/2, 2],
                  [ex+ew-H/2, ey+eh-H/2, 3],
                ];
                return (
                  <>
                    {/* Selection border */}
                    ...
                    {/* Size label */}
                    <div style={{ position:"absolute", left:ex, top:ey+eh+6, fontFamily:DM, fontSize:10, color:"#3ECFCF", background:"rgba(0,0,0,0.55)", padding:"1px 5px", borderRadius:4, pointerEvents:"none", zIndex:901, whiteSpace:"nowrap" }}>
                      {Math.round(selectedLT.w)} × {Math.round(selectedLT.h)}
                    </div>
                    {/* Resize corner handles */}
                    {handles.map(([hx,hy,idx]) => (
                      <div key={idx}
                        style={{ position:"absolute", left:hx, top:hy, width:H, height:H, background:"#fff", border:"2px solid #3ECFCF", zIndex:902, cursor:CURSORS[idx], pointerEvents:"all" }}
                        onMouseDown={(e) => handleCornerMouseDown(e, selectedEl, idx)} />
                    ))}

                    {/* Toolbar */}
                    <div style={{ position:"absolute", left:toolbarX, top:toolbarY, zIndex:950, background:"#141414", border:"1px solid #333", borderRadius:14, padding:"8px 10px", display:"flex", flexDirection:"column", gap:7, boxShadow:"0 8px 28px rgba(0,0,0,0.8)", minWidth:320, pointerEvents:"all" }}
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                        ...
                        <div style={{ position:"relative" }}>
                          <button title="Fill Color" onClick={() => setShowColorPicker((v) => !v)}
                            style={{ width:26, height:26, borderRadius:"50%", background:selectedEl.fill==="transparent"?"linear-gradient(135deg,#f55,#5af,#5f5)":selectedEl.fill, border:"2px solid #444", cursor:"pointer", flexShrink:0 }} />
                          {showColorPicker && (
                            <div style={{ position:"absolute", bottom:32, left:0, background:"#0e0e0e", border:"1px solid #333", borderRadius:10, padding:8, display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:4, zIndex:960, boxShadow:"0 8px 28px rgba(0,0,0,0.9)" }}>
                              {PALETTE.map((c) => (
                                <button key={c} onClick={() => { onUpdate(selectedEl.id,{fill:c}); setShowColorPicker(false); }}
                                  style={{ width:22, height:22, borderRadius:"50%", background:c, border:selectedEl.fill===c?"2.5px solid #3ECFCF":"1.5px solid #444", cursor:"pointer", flexShrink:0 }} />
                              ))}
                            </div>
                          )}
                        </div>
                      ...
                      {/* Row 2: size + corner radius + opacity */}
                      <div style={{ display:"flex", alignItems:"center", gap:5, borderTop:"1px solid #222", paddingTop:7 }}>
                        <span style={{ fontFamily:DM, fontSize:"0.6rem", color:"#555", flexShrink:0 }}>W</span>
                        <input type="number" value={Math.round(selectedLT.w)} min={MIN_W} max={CANVAS_W}
                          onChange={(e) => onUpdate(selectedEl.id,{width:Number(e.target.value)})}
                          style={inputStyle} />
                        <span style={{ fontFamily:DM, fontSize:"0.6rem", color:"#555", flexShrink:0 }}>H</span>
                        <input type="number" value={Math.round(selectedLT.h)} min={MIN_H} max={CANVAS_H}
                          onChange={(e) => onUpdate(selectedEl.id,{height:Number(e.target.value)})}
                          style={inputStyle} />
                        <span style={{ fontFamily:DM, fontSize:"0.6rem", color:"#555", flexShrink:0 }}>R</span>
                        <input type="number" value={selectedEl.cornerRadius??0} min={0} max={100}
                          onChange={(e) => onUpdate(selectedEl.id,{cornerRadius:Number(e.target.value)})}
                          title="Corner Radius"
                          style={inputStyle} />
                        <span style={{ fontFamily:DM, fontSize:"0.6rem", color:"#555", flexShrink:0 }}>%</span>
                        <input type="number" value={Math.round((selectedEl.opacity??1)*100)} min={0} max={100}
                          onChange={(e) => onUpdate(selectedEl.id,{opacity:Number(e.target.value)/100})}
                          title="Opacity"
                          style={inputStyle} />
                      </div>

                      {/* Row 3: alignment to canvas */}
                      <div style={{ display:"flex", alignItems:"center", gap:3, borderTop:"1px solid #222", paddingTop:7 }}>
                        <span style={{ fontFamily:DM, fontSize:"0.58rem", color:"#444", marginRight:2, flexShrink:0 }}>Align</span>
                        {([
                          ["⊢","lx","Left edge"],["↔","cx","Center H"],["⊣","rx","Right edge"],
                          ["⊤","ty","Top edge"],["↕","cy","Center V"],["⊥","by","Bottom edge"],
                        ] as [string,string,string][]).map(([icon,axis,title]) => (
                          <ToolBtn key={axis} title={title} onClick={() => alignEl(axis as "lx"|"cx"|"rx"|"ty"|"cy"|"by")}>
                      ...
                      {/* Row 4: text controls */}
                      {isTextEl && (
                        <div style={{ display:"flex", alignItems:"center", gap:4, borderTop:"1px solid #222", paddingTop:7 }}>
                          <select value={selectedEl.fontSize??14} onChange={(e) => onUpdate(selectedEl.id,{fontSize:Number(e.target.value)})}
                            style={{ background:"#1c1c1c", border:"1px solid #444", color:"#ddd", borderRadius:6, padding:"2px 4px", fontFamily:DM, fontSize:"0.7rem", cursor:"pointer" }}>
...
const inputStyle: React.CSSProperties = {
  width:44, background:"#1c1c1c", border:"1px solid #444", color:"#ccc",
...
function ToolBtn({ children, onClick, title }: { children:React.ReactNode; onClick:()=>void; title?:string }) {
  return (
    <button title={title} onClick={onClick}
      style={{ background:"rgba(255,255,255,0.07)", border:"1px solid #333", color:"#ddd", borderRadius:7, width:28, height:28, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, flexShrink:0 }}
